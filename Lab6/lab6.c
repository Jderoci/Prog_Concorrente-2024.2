...
void EntraLeitura() {
  ...
  while (escr > 0 || querEscrever > 0) { // Espera se houver escritores ativos ou na fila
...
void SaiLeitura() {
  ...
  if (leit == 0) {
    ...
    pthread_cond_signal(&cond_escr); // Notifica escritores se não houver leitores ativos
  }
  pthread_mutex_unlock(&mutex);
}
...
void EntraEscrita() {
  ...
  while (leit > 0 || escr > 0) { // Espera enquanto há leitores ou escritores ativos
    printf("Aguardando leitores ou escritores ativos.. (Leitores ativos: %d, Escritores ativos: %d)\n", leit, escr);
...
void SaiEscrita() {
  ...
  if (querEscrever > 0) {
    ...
    pthread_cond_signal(&cond_escr); // Dá prioridade a outros escritores na fila
  } else {
    printf("Nenhum escritor pendente. Sinalizando leitores..\n");
    pthread_cond_broadcast(&cond_leit); // Notifica leitores se não houver mais escritores
  }
  pthread_mutex_unlock(&mutex);
}
